...
module.exports = function exports(options) {
  ...
  const init = function init(cb) {
    ...
    // });
    async.series([
      bus.open.bind(null, options.busnum, {}),
      (cb2) => {
        debug('Reseting PCA9685 MODE1 (without SLEEP) and MODE2'); cb2(null, null);
      },
      setAllPWM.bind(null, 0, 0),
      bus.writeByte.bind(null, options.address, registers.MODE2, bits.OUTDRV),
      bus.writeByte.bind(null, options.address, registers.MODE1, bits.ALLCALL),
    ], (err3) => {
      if (err3) return cb(err3);
      setTimeout(() => {
        async.waterfall([
          bus.readByte.bind(null, options.address, registers.MODE1),
          (byte, cb3) => {
            // wake up(reset sleep));
